use sentinel_domain::{
    ...
    ids::{ApprovalId, PolicyPackId, PrincipalId, TenantId},
...
fn test_workflow_state_machine_happy_path() {
    ...
    let rev_id = sentinel_domain::ids::RevisionId("sha256:1234".to_string());
    let mut wf = Workflow::new(
        tenant_id,
        rev_id,
        PolicyPackId::new("ap-v1"),
        "v1".to_string(),
    );
    ...
    let state = wf
        .apply(WorkflowCommand::EvidenceCollected, 3, now)
        .unwrap();
    ...
    let state = wf
        .apply(WorkflowCommand::RetestFinished { passed: true }, 7, now)
        .unwrap();
