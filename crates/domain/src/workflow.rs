...
impl Workflow {
    ...
    pub fn apply(
        ...
        if self.state.is_terminal() {
            ...
            if let (WorkflowState::Certified, WorkflowCommand::Revoke { .. }) =
                (&self.state, &command)
            {
        ...
        let next_state = match (&self.state, &command) {
            ...
            (WorkflowState::Running, WorkflowCommand::EvidenceCollected) => {
            ...
            (WorkflowState::RetestRequired, WorkflowCommand::DispatchRetest) => {
            ...
            (
                WorkflowState::Registered | WorkflowState::Queued,
                WorkflowCommand::Cancel { reason },
            ) => {
                self.gate_explanation = Some(reason.clone());
