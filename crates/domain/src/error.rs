...
impl From<TransitionError> for DomainError {
    fn from(value: TransitionError) -> Self {
        match value {
            TransitionError::Illegal { from, command, .. } => DomainError::InvalidTransition { from, command },
            TransitionError::Terminal(state) => DomainError::Invalid(format!("terminal state {state:?}")),
