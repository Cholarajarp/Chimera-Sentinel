...
use sentinel_domain::{
    attestation::{AttestationMetadata, AttestationPayload, AttestationStatus, KeyReference, VerificationChecks, VerificationResult},
...
pub async fn sign_attestation(
    ...
    {
        match kms_sign_async(&key_ref.key_resource, &key_ref.key_version, &canonical_bytes).await {
            Ok((sig, ver)) => (sig, ver),
            Err(e) => {
...
async fn get_gcp_access_token() -> Result<String, String> {
    ...
    let creds_path = std::env::var("GOOGLE_APPLICATION_CREDENTIALS")
        .map_err(|_| "Neither metadata server nor GOOGLE_APPLICATION_CREDENTIALS available".to_string())?;
    ...
    let token = exchange_service_account_jwt(client_email, private_key, token_uri, "https://www.googleapis.com/auth/cloudkms").await?;
    Ok(token)
}
...
pub fn verify_attestation(
    ...
    let is_local_key = !attestation.key_reference.key_resource.starts_with("projects/")
    ...
    if is_local_key {
        ...
        // fetching the public key from KMS. For the demo verifier, we check
        ...
        if !attestation.signature.is_empty() && attestation.signature.len() > 32 {
            ...
            errors.push("Signature field is missing or too short for a Cloud KMS RSA-PSS signature");
        }
    }

    // 3. Key Reference Validation
    if !attestation.key_reference.key_resource.is_empty() && !attestation.key_reference.key_version.is_empty() {
    ...
    // 7. Environment
    if attestation.payload.environment.eq_ignore_ascii_case(expected_env) {
    ...
    checks.digest_matches = !attestation.payload.evidence_manifest_digest.is_empty()
        && attestation.payload.evidence_manifest_digest.starts_with("sha256:");
    ...
    VerificationResult {
        valid,
        payload: if valid { Some(attestation.payload.clone()) } else { None },
