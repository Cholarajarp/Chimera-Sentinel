...
pub trait CandidateRepository: Send + Sync {
    ...
    async fn list(&self, tenant_id: TenantId, limit: usize, cursor: Option<String>) -> Result<Vec<CandidateRevision>, String>;
}
...
pub mod memory {
    ...
    impl AuditRepository for InMemoryStore {
        ...
        async fn query(&self, tenant_id: TenantId, workflow_id: Option<WorkflowId>, limit: usize) -> Result<Vec<AuditEvent>, String> {
            ...
            let mut results: Vec<_> = data
                .iter()
                .filter(|e| e.tenant_id == tenant_id && workflow_id.map_or(true, |wid| e.workflow_id == Some(wid)))
                .cloned()
                .collect();
    ...
    impl AttestationRepository for InMemoryStore {
        async fn store(&self, workflow_id: WorkflowId, attestation: &AttestationMetadata) -> Result<(), String> {
            ...
            data.insert((attestation.payload.tenant_id, workflow_id), attestation.clone());
            Ok(())
        }
