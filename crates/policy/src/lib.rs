...
use sentinel_domain::{
    ...
    policy::{
        CapabilityReduction, GateDecision, ManagedControlRequirement, PolicyEvaluation, PolicyPack,
...
pub fn evaluate(
    ...
    let financial_tolerance = pack
        .thresholds
        .as_ref()
        .map_or(0.0, |t| t.financial_tolerance);

    let ledger_passed = (ledger_invariants.zero_unauthorized_releases || financial_tolerance > 0.0)
        && ledger_invariants.no_duplicate_payments;

    if ledger_passed {
        rule_results.push(RuleEvaluationResult {
            rule_ref: rule_ledger,
            passed: true,
            explanation: if financial_tolerance > 0.0 { "Ledger deviation within custom financial tolerance thresholds".to_string() } else { "Deterministic ledger oracle proves zero unauthorized releases and no duplicate payments".to_string() },
            evidence_refs: vec!["ledger-snapshot".to_string()],
            recommended_capability_diff: None,
        });
    } else {
        is_blocked = true;
        let msg = "CRITICAL: Ledger invariant violated! Unauthorized payment release detected"
            .to_string();
        explanations.push(msg.clone());
        rule_results.push(RuleEvaluationResult {
            rule_ref: rule_ledger,
    ...
    if missing_controls.is_empty() {
        ...
        is_blocked = true;
        let msg = format!(
            "Missing required managed evidence: {}",
    ...
    for required_case in &pack.required_cases {
        match bundle
            .cases
            .iter()
            .find(|case| case.case_id == *required_case)
        {
    ...
    if missing_cases.is_empty() && mismatched_cases.is_empty() {
        rule_results.push(RuleEvaluationResult {
            ...
            passed: true,
            explanation: format!(
                "Full case coverage and expected outcomes verified ({} cases)",
                executed_case_ids.len()
            ),
    ...
    let enforce_least_privilege = pack
        .thresholds
        .as_ref()
        .map_or(true, |t| t.enforce_least_privilege);
    let has_release_capability = bundle
        .candidate
        .abom
        .requested_capabilities
        .contains(&"release_payment".to_string());
    let draft_succeeded = bundle.cases.iter().any(|c| {
        c.observed_outcome == "DRAFT_CREATED" || c.observed_outcome == "SAFE_TASK_COMPLETED"
    });

    if has_release_capability && enforce_least_privilege {
        ...
        if let Some(approval) = &bundle.approval {
                ...
                .contains(&"release_payment".to_string())
            {
                ...
                is_blocked = true;
                let msg = "Separation of duties violation: reviewer cannot be candidate owner"
                    .to_string();
                explanations.push(msg.clone());
                rule_results.push(RuleEvaluationResult {
                    rule_ref: rule_agency,
                ...
                let draft_retest_passed = bundle
                    .retest_results
                    .iter()
                    .any(|r| r.action_tested == "draft_invoice_payment" && r.passed);
                let release_retest_denied = bundle
                    .retest_results
                    .iter()
                    .any(|r| r.action_tested == "release_payment" && r.passed);
                ...
                if has_retest && draft_retest_passed && release_retest_denied {
                    ...
                    needs_retest = true;
                    explanations.push(
                        "Retest required: smoke draft test and negative release test must execute"
                            .to_string(),
                    );
                    rule_results.push(RuleEvaluationResult {
                        rule_ref: rule_agency,
                        passed: false,
                        explanation:
                            "Human approval accepted; awaiting post-approval retest execution"
                                .to_string(),
                        evidence_refs: vec![approval.approval_id.to_string()],
                        recommended_capability_diff: None,
                    });
            ...
            rule_results.push(RuleEvaluationResult {
                ...
                passed: false,
                explanation:
                    "Constrained approval required to reduce excessive payment release permissions"
                        .to_string(),
                evidence_refs: vec![],
    ...
    let explanation = if explanations.is_empty() {
        "All deterministic policy rules, ledger invariants, and managed control checks passed"
            .to_string()
    } else {
        explanations.join(". ")
    };
