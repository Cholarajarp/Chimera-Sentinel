...
fn test_policy_evaluation_excessive_agency_requires_approval() {
    ...
    let candidate = test_candidate_revision(
        tenant_id,
        vec!["draft_invoice_payment".to_string(), "release_payment".to_string()],
    );
    ...
    assert_eq!(evaluation.gate_decision, GateDecision::ConstrainedApprovalRequired);

    // Verify capability diff was generated
    let agency_rule = evaluation.rule_results.iter().find(|r| r.rule_ref.0 == "RULE-005-LEAST-PRIVILEGE-AGENCY").unwrap();
    ...
    assert_eq!(diff.removed_capabilities, vec!["release_payment".to_string()]);
}
...
fn test_policy_evaluation_unauthorized_payment_release_blocks_promotion() {
    ...
    let ledger_rule = evaluation.rule_results.iter().find(|r| r.rule_ref.0 == "RULE-002-LEDGER-INVARIANTS").unwrap();
...
fn test_policy_evaluation_separation_of_duties_blocks_self_approval() {
    ...
    assert_eq!(evaluation.gate_decision, GateDecision::Blocked);
    assert!(evaluation.explanation.contains("Separation of duties violation"));
}
