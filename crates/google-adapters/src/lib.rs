...
pub mod model_armor {
    ...
    pub async fn inspect_content(
        ...
        let (disposition, matched_rules, explanation) = if content.to_lowercase().contains("ignore previous instructions")
            || content.to_lowercase().contains("ignore policy")
            || content.to_lowercase().contains("override policy")
            || content.to_lowercase().contains("pay immediately without review")
        {
            (
                "BLOCK".to_string(),
                vec!["PROMPT_INJECTION_DIRECT".to_string()],
                "Model Armor detected direct prompt injection attempting policy override".to_string(),
            )
        } else {
            (
                "ALLOW".to_string(),
        ...
        Ok(ModelArmorEvidence {
            ...
            provider_reference: format!("projects/sentinel-prod/locations/us-central1/templates/{}", template_id),
            provenance: if is_live { Provenance::Live } else { Provenance::Local },
            timestamp: now,
        })
    }
}
...
pub mod gateway {
    ...
    pub async fn check_permission(
        ...
        Ok(GatewayDecisionEvidence {
            ...
            provider_reference: format!("projects/sentinel-prod/locations/us-central1/gateways/gw-{}", tenant_id),
            provenance: if is_live { Provenance::Live } else { Provenance::Local },
            timestamp: now,
        })
    }
...
pub mod memory_bank {
    ...
    pub async fn query_advisory_context(
        ...
        Ok(MemoryRetrievalEvidence {
            ...
            provider_reference: format!("projects/sentinel-prod/locations/us-central1/memoryBanks/{}", memory_bank_id),
            provenance: if is_live { Provenance::Live } else { Provenance::Local },
            timestamp: now,
        })
    }
