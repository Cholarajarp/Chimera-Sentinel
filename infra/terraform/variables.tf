...
variable "hot_path_min_instances" {
  description = "Warm instances for control-plane, ADK certifier, and Enterprise ERP Adapter. Use 1 for recorded demos; keep 0 for lowest idle cost."
  type        = number
  default     = 0
...
variable "gemini_model_ref" {
  ...
  type        = string
  default     = "gemini-3.5-flash-lite" # GA: cheap, fast, high-volume workloads
}

variable "gemini_model_ref_agent" {
  description = "ADK agent orchestration model — certifier brain, tool-intent extraction"
  type        = string
  default     = "gemini-3.5-flash" # GA: high-end agentic/coding workloads
}
