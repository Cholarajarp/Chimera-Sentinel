...
resource "google_service_account" "mock_erp" {
  ...
  display_name = "Sentinel Mock ERP MCP"
}
...
locals {
  sa_roles = {
    ...
    "serviceAccount:${google_service_account.workflow_worker.email}" = [
      "roles/datastore.user",
      "roles/storage.objectUser",
      "roles/cloudkms.signer",
      "roles/cloudkms.signerVerifier",
    ...
    "serviceAccount:${google_service_account.adk_certifier.email}" = [
...
resource "google_project_iam_member" "sa_bindings" {
  for_each = merge([
    for sa, roles in local.sa_roles : {
      for role in roles : "${sa}__${role}" => { member = sa, role = role }
    }
  ]...)
...
resource "google_cloud_run_v2_service_iam_member" "adk_certifier_internal" {
  service  = google_cloud_run_v2_service.adk_certifier.name
  location = var.region
  role     = "roles/run.invoker"
  member   = "serviceAccount:${google_service_account.workflow_worker.email}"
}

# ── Cloud Run: Mock ERP MCP ───────────────────────────────────────────────────
resource "google_cloud_run_v2_service" "mock_erp" {
  name     = "sentinel-mock-erp-mcp"
  location = var.region
  ...
  template {
    ...
    containers {
      image = "${var.region}-docker.pkg.dev/${var.project_id}/sentinel/mock-erp-mcp:latest"
...
resource "google_cloud_run_v2_service_iam_member" "mock_erp_internal" {
  service  = google_cloud_run_v2_service.mock_erp.name
  location = var.region
  role     = "roles/run.invoker"
...
resource "google_cloud_run_v2_service" "web" {
  ...
  template {
    ...
    scaling {
      # Keep 1 warm instance so the demo never cold-starts for a judge.
      min_instance_count = var.hot_path_min_instances
      max_instance_count = 2
    }
