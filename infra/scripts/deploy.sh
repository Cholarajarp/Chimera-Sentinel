...
# ── 3. Build & push enterprise-erp-adapter ──────────────────────────────────────────────
echo "[5/8] Building sentinel-enterprise-erp-adapter..."
docker build \
  -f apps/enterprise-erp-adapter/Dockerfile \
  -t "${REPO}/enterprise-erp-adapter:${GIT_SHA}" \
  -t "${REPO}/enterprise-erp-adapter:latest" \
  .
docker push "${REPO}/enterprise-erp-adapter:${GIT_SHA}"
docker push "${REPO}/enterprise-erp-adapter:latest"
...
# Extract Firebase vars from tfvars to bake into the Next.js static bundle
get_tfvar() {
  grep "^$1" infra/terraform/terraform.tfvars | grep -o '".*"' | tr -d '"' || true
}

docker build \
  -f apps/web/Dockerfile \
  --build-arg NEXT_PUBLIC_SITE_URL="https://${PROJECT_ID}.web.app" \
  --build-arg NEXT_PUBLIC_FIREBASE_API_KEY="$(get_tfvar firebase_api_key)" \
  --build-arg NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN="$(get_tfvar firebase_auth_domain)" \
  --build-arg NEXT_PUBLIC_FIREBASE_PROJECT_ID="$(get_tfvar firebase_project_id)" \
  --build-arg NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET="$(get_tfvar firebase_storage_bucket)" \
  --build-arg NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID="$(get_tfvar firebase_messaging_sender_id)" \
  --build-arg NEXT_PUBLIC_FIREBASE_APP_ID="$(get_tfvar firebase_app_id)" \
  --build-arg NEXT_PUBLIC_FIREBASE_MEASUREMENT_ID="$(get_tfvar firebase_measurement_id)" \
  -t "${REPO}/web:${GIT_SHA}" \
  -t "${REPO}/web:latest" \
  .
...
# Terraform ignores updates to the ':latest' tag because the string itself
# doesn't change. We must explicitly tell Cloud Run to roll out the new GIT_SHA.
echo "Rolling out new container images to Cloud Run..."
...
gcloud run services update sentinel-enterprise-erp-adapter --image "${REPO}/enterprise-erp-adapter:${GIT_SHA}" --region="${REGION}" --project="${PROJECT_ID}" --quiet || true
gcloud run services update sentinel-adk-certifier --image "${REPO}/adk-certifier:${GIT_SHA}" --region="${REGION}" --project="${PROJECT_ID}" --quiet || true
