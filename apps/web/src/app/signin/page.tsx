...
export default function SignInPage() {
  ...
  const enterDemo = () => {
    ...
    // Secure flag is added automatically by browsers on HTTPS; omit on localhost
    const secure = window.location.protocol === 'https:' ? '; Secure' : '';
    document.cookie = `sentinel-session=demo; path=/; max-age=7200; SameSite=Strict${secure}`;
  ...
  const handleOAuth = async (provider: 'google' | 'github') => {
    ...
    try {
      ...
      if (code === 'auth/popup-closed-by-user' || code === 'auth/cancelled-popup-request') {
        ...
        setAuthError('Sign-in failed. Please try again or use the guided demo.');
      }
    } finally {
      setLoading(null);
    }
    ...
    <main className="signin-page">
      ...
      <section className="signin-shell" aria-labelledby="signin-title">
        ...
        <div className="signin-options">
          ...
          <div className="signin-divider"><span>Judge access</span></div>
          <button className="signin-demo" type="button" onClick={enterDemo}>
            <span className="signin-option-icon"><CheckCircle2 size={19} /></span>
            <span><strong>Open guided judge demo</strong><small>No account required · evidence clearly labeled replay</small></span>
            <ArrowRight size={16} />
          </button>

          <p className="signin-policy">Live Cloud mode verifies API health before allowing execution. Guided replay never claims cloud provenance.</p>
        </div>
      </section>
    </main>
  );
