...
- Gemini 3.5 Flash Lite  — high-volume batch case evaluation (80 cases per run)
- Gemini 3.5 Flash       — ADK agent orchestration / certifier brain
- Gemini 3.1 Pro         — deep analysis for ambiguous / escalated cases
...
class CaseObservation(BaseModel):
    ...
    model_armor_disposition: Optional[str] = None
    model_armor_provider_reference: Optional[str] = None
    candidate_identity: str
    requested_tool: Optional[str] = None
    gateway_decision: Optional[str] = None
    gateway_provider_reference: Optional[str] = None
    ...
    gemini_model_ref: Optional[str] = None
    provenance: str = "LIVE"
...
class EvaluationBatchResult(BaseModel):
    ...
    ledger_snapshot_before: Optional[dict[str, Any]] = None
    ledger_snapshot_after: Optional[dict[str, Any]] = None
    gemini_model_ref: Optional[str] = None        # batch / flash-lite model
    gemini_model_ref_agent: Optional[str] = None  # ADK agent / flash model
    gemini_model_ref_deep: Optional[str] = None   # deep analysis / pro model
...
def _load_corpus_case(case_id: str) -> Optional[dict[str, Any]]:
...
async def _model_armor_inspect(
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            filter_state = (
                data.get("sanitizationResult", {})
                .get("filterMatchState", "NOT_MATCHED")
            )
...
async def _gateway_check_permission(
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            logger.info("Agent Gateway decision=%s for principal=%s action=%s", decision, principal, action)
...
async def execute_case_live(
    ...
    google_project: str,
    google_region: str,
    gemini_model_ref: str,          # batch evaluation model (gemini-3.5-flash-lite)
    gemini_model_ref_agent: str,    # ADK agent orchestration model (gemini-3.5-flash)
    gemini_model_ref_deep: str,     # deep analysis model (gemini-3.1-pro)
    ...
    Model routing:
    - gemini-3.5-flash-lite → high-volume batch case evaluation (80 cases)
    - gemini-3.5-flash       → ADK LlmAgent orchestration (tool intent)
    - gemini-3.1-pro         → deep analysis / ambiguous case escalation
    ...
    if armor_disposition == "BLOCK":
        ...
        return CaseObservation(
            tenant_id=tenant_id, workflow_id=workflow_id, case_run_id=case_run_id,
            case_id=case_id, corpus_version=corpus_version,
            expected_outcome=expected_outcome, observed_outcome="PREVENTED_AT_ARMOR",
            model_armor_disposition="BLOCK", model_armor_provider_reference=armor_provider_ref,
            candidate_identity=candidate_identity, requested_tool=None, gateway_decision=None,
            ledger_snapshot_before_digest=before_digest,
            ledger_snapshot_after_digest=before_digest,
            latency_ms=elapsed, token_count=0, cost_usd_micro=5,
            trace_id=trace_id, gemini_model_ref=gemini_model_ref,
            provenance=provenance, timestamp=now_iso,
        )

    if armor_disposition == "SERVICE_ERROR":
        raise RuntimeError(f"Model Armor unavailable for case {case_id} — admission blocked (fail closed)")
    ...
    if gateway_decision == "ALLOW" and requested_tool == "draft_invoice_payment":
        # Allowed — execute the tool against the Mock ERP
    ...
    return CaseObservation(
        tenant_id=tenant_id, workflow_id=workflow_id, case_run_id=case_run_id,
        case_id=case_id, corpus_version=corpus_version,
        expected_outcome=expected_outcome, observed_outcome=observed_outcome,
        ...
        latency_ms=elapsed, token_count=token_count, cost_usd_micro=cost_usd_micro,
        trace_id=trace_id,
...
async def _run_adk_agent(
    ...
    runner = Runner(agent=agent, app_name="sentinel-certifier", session_service=session_service)
    ...
    requested_tool = None
    try:
        clean = last_text.strip().lstrip("```json").rstrip("```").strip()
        parsed = json.loads(clean)
        requested_tool = parsed.get("requested_tool")
    except (json.JSONDecodeError, AttributeError, ValueError):
        logger.warning("ADK model response not parseable as JSON — Gateway/ledger remain authoritative")
    ...
    logger.info(
        ...
        model_name, token_count, requested_tool,
    )
    return {
        "requested_tool": requested_tool,
...
async def _execute_erp_tool(
    ...
    return _sha256_json({"tenant": tenant_id, "case": case_id, "phase": "after", "ts": _time.time()})
...
def execute_case(
    ...
    if case_def is not None:
        ...
        if "safe-draft" in case_id or "safe-invoice" in case_id:
            ...
            expected_armor, expected_gateway, requested_tool = "ALLOW", "ALLOW", "draft_invoice_payment"
